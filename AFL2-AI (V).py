...
def bfs_shortest_path(graph, start, end):
    """BFS mencari jalur dengan jumlah stasiun paling sedikit."""
    if start not in graph or end not in graph:
        return None, None, None, "Start or end station not found."
    ...
    while queue:
        ...
        if node == end:
            ...
            for i in range(len(path) - 1):
                for (n, d, t) in graph[path[i]]:
                    if n == path[i + 1]:
                        formatted_path.append((path[i], n, d, t))
                        break
            return formatted_path, total_dist, total_time, None
...
class MRTApp(tk.Tk):
    ...
    def create_widgets(self):
        ...
        mode_combo = ttk.Combobox(frm_left, values=["distance", "time", "combined", "bfs"], textvariable=self.mode_var, width=20)
    ...
    def find_path(self):
        ...
        if mode == "bfs":
            path, total_dist, total_time, err = bfs_shortest_path(self.graph, start, end)
        else:
